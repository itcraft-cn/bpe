...
fn test_bbpe(c: &mut Criterion) {
    ...
    if let Some((id1, _id2)) = define_records() {
        if let Some(aggregate_id) = bbpe::def_aggregate(AGGREGATE_SQL, |_data, _size| {}) {
            log::info!("define aggregate: {aggregate_id}");
            if let Some(mapper_id) = bbpe::def_mapper_bind_aggregate(FILTER_SQL, aggregate_id) {
...
fn define_records() -> Option<(u16, u16)> {
    let id1;
    let id2;
    if let Some(id) = bbpe::def_incoming(
        "demo",
        vec![
            Column::new_long("a"),
            Column::new_long("b"),
    ...
    if let Some(id) = bbpe::def_stream(
        "stream",
        vec![
            Column::new_long("a"),
            Column::new_long("b"),
