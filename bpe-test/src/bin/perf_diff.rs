...
fn main() {
    ...
    start();
    let rid = def_incoming("pd_demo", vec![
    ...
    def_mapper("SELECT pd_demo.a, pd_demo.b FROM pd_demo WHERE pd_demo.amount > 900000000000000000 LIMIT 10", |_p| {}).unwrap();
    let c = bench("C insert+filter(always-false)", || {
        i += 1;
        unsafe { *(ptr.add(16) as *mut i64) = 1000 + (i % 100) };
        new_data(&u8data);
    });
    def_mapper("SELECT pd_demo.user_id, _to_double(pd_demo.amount), _add(pd_demo.a, pd_demo.b), _sub(pd_demo.c, pd_demo.d), _abs(pd_demo.amount) \
         FROM pd_demo WHERE pd_demo.amount > 100 LIMIT 10", |_p| {}).unwrap();
    ...
    eprintln!("insert:           {a:.1}");
    eprintln!("filter+scan:      {:.1}   (C - A)", c - a);
