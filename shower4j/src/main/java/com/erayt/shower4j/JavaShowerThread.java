package com.erayt.shower4j;

import org.jctools.queues.MessagePassingQueue;
import org.jctools.queues.atomic.MpscAtomicArrayQueue;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.concurrent.atomic.AtomicBoolean;

/**
 * @author Helly Guo
 * <p>
 * Created on 11/21/23 10:51 AM
 */
class JavaShowerThread {

    private static final Logger LOGGER = LoggerFactory.getLogger(JavaShowerThread.class);

    private final AtomicBoolean active = new AtomicBoolean(true);
    private final Thread thread;
    private final MpscAtomicArrayQueue<WrappedData<?>> queue = new MpscAtomicArrayQueue<>(10240);

    public JavaShowerThread() {
        this.thread = new Thread(this::runShower, "java-shower-thread");
        this.thread.setDaemon(true);
    }

    public void fillQueue(WrappedData<?> data) {
        queue.offer(data);
    }

    private void runShower() {
        MessagePassingQueue.Consumer<WrappedData<?>> dataConsumer = this::consumeData;
        MessagePassingQueue.WaitStrategy waitStrategy = (counter) -> counter + 1;
        MessagePassingQueue.ExitCondition exitCondition = active::get;
        while (active.get()) {
            queue.drain(dataConsumer, waitStrategy, exitCondition);
        }
    }

    private <T> void consumeData(WrappedData<T> data) {
        boolean success = JavaShower.newData(data.getId(), data.getData());
        data.getFuture().complete(success);
    }

    public void start() {
        this.thread.start();
    }

    public void stop(long timeout) {
        active.set(false);
        try {
            this.thread.join(timeout);
        } catch (InterruptedException e) {
            LOGGER.warn("interrupted", e);
        }
    }
}
