 ...
 */
class JavaBambooTubeThread {

    private static final Logger LOGGER = LoggerFactory.getLogger(JavaBambooTubeThread.class);

    private final AtomicBoolean active = new AtomicBoolean(true);
    private final Thread thread;
    private final MpscAtomicArrayQueue<WrappedData<?>> queue = new MpscAtomicArrayQueue<>(10240);

    public JavaBambooTubeThread() {
        this.thread = new Thread(this::runBambooTube, "java-bambootube-thread");
    ...
    private <T> void consumeData(WrappedData<T> data) {
        boolean success = JavaBambooTube.newData(data.getId(), data.getData());
