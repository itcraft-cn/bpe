...
fn test_bpe(c: &mut Criterion) {
    ...
    if let Some((id1, _id2)) = define_records() {
        if let Some(aggregate_id) = bpe::def_aggregate(AGGREGATE_SQL, |_params| {}) {
            log::info!("define aggregate: {aggregate_id}");
            if let Some(mapper_id) = bpe::def_mapper_bind_aggregate(FILTER_SQL, aggregate_id) {
        ...
        test(c, "new_data_and_filter_and_aggregate", |b| {
            b.iter(|| bpe::new_data(&u8data))
        });
    }
}
...
fn define_records() -> Option<(u16, u16)> {
    let id1;
    let id2;
    if let Some(id) = bpe::def_incoming(
        "demo",
        vec![
            Column::new_long("a"),
            Column::new_long("b"),
