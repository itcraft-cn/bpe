...
class JavaBpeThread {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(JavaBpeThread.class);

    private final AtomicBoolean active = new AtomicBoolean(true);
    private final Thread thread;
    private final MpscAtomicArrayQueue<WrappedData<?>> queue = new MpscAtomicArrayQueue<>(10240);

    public JavaBpeThread() {
        this.thread = new Thread(this::runBpe, "java-bpe-thread");
    ...
    private <T> void consumeData(WrappedData<T> data) {
        boolean success = JavaBpe.newData(data.getId(), data.getData());
