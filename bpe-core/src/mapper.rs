...
fn loop_filter(
    ...
    loop {
        ...
        if unsafe { filter.call(v_sub_ptr) } {        // Apply the filter
            ...
            if n == limit {      // Check if we've reached the limit
                break;
            }
        }
        if idx_wrapper.judge_or_step() {  // Check if we should continue iterating
            break;
        }
    }
