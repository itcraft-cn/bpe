...
fn build_float_cmp<'ctx>(
    context: &GenContext<'ctx>,      // Generation context with LLVM builder and types
    _float_op: FloatPredicate,      // Float comparison predicate (currently unused)
    ...
    let _l_float_val = convert_int2float(context, "l", l_val_type, l_val);  // Convert left value
    let _r_float_val = convert_int2float(context, "r", r_val_type, r_val);  // Convert right value
    let float_result_int = i64_type.const_int(T_B64, true);  // Boolean result constant
