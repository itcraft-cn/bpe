...
#[inline]
fn func_callback(sum_store: &Arc<AtomicI64>, data: *const u8, _size: usize) {
    let now = now();
    let timestamp = fetch_f64(data);
    let _timestamp_min = fetch_f64(data.wrapping_add(8));
    let _timestamp_sum = fetch_f64(data.wrapping_add(16));
    let _const_val = fetch_i64(data.wrapping_add(24));
