...
fn new_data(id: u16, bdata: &[u8]) -> PyResult<bool> {
    ...
    Ok(bbpe::new_data(&data))
}

#[pyfunction]
#[allow(dead_code)]
fn def_mapper(sql: &str, callback: PyObject) -> PyResult<i32> {
    def_action(sql, callback, bbpe::def_mapper_ffi)
}

#[pyfunction]
#[allow(dead_code)]
fn def_mapper_bind_aggregate(sql: &str, aggregate_id: u16) -> PyResult<i32> {
    if let Some(id) = bbpe::def_mapper_bind_aggregate(sql, aggregate_id) {
        Ok(id as i32)
    } else {
        Ok(-1)
    }
...
fn def_aggregate(sql: &str, callback: PyObject) -> PyResult<i32> {
    def_action(sql, callback, bbpe::def_aggregate_ffi)
}
...
#[pymodule]
#[allow(dead_code)]
fn bbpe4py(_py: Python, m: &PyModule) -> PyResult<()> {
